...
export default class FindAParkScreen extends React.Component {
  ...
  getAmenityPaths(){
    HELPER.timeout(ERRORS.timeout, fetch('https://locator.lacounty.gov/parks/api/TagList?tagType=park%20things%20to%20do'))
  ...
  computeTotalParks() {
    let total = 0
    let chosen = Array.from(this.state.amenitiesChosen);
  ...
  startSearch() {
    ...
    this.props.navigation.push('ParksFound', {
      ...
      amenities: Array.from(this.state.amenitiesChosen),
      count: this.computeTotalParks()
    })
  }
  ...
  render() {
    return (
      <ImageBackground source={require('./../../../assets/backgrounds/Programs.jpg')} style={{width: '100%', height: '100%'}}>
